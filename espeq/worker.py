...
class Worker:
    ...
    def _run(self):
        pid = None
        for _ in range(self.redq.concurrency):
            pid = self._fork()
    ...
    def _parent(self):
        ...
        self.redq.broker.close()
        pubsub = self.redq.broker.pubsub()
        pubsub.subscribe(self.redq.broadcast_key)
        while True:
            msg = pubsub.get_message(ignore_subscribe_messages=True, timeout=10)
            if msg:
                queue_broker_key, payload = self._handle_pub_message(msg)
                self.redq.broker.lpush(queue_broker_key, payload)

    def _handle_pub_message(self, msg):
        payload = deserialize(msg["data"])
        queue = Queue.create(
            payload.pop("queue"), queues_by_name=self.redq.queues_by_name
        )
    ...
    def _child(self):
        ...
        self.redq.broker.connection_pool.reset()
    ...
    def _enqueue_ready_eta_tasks(self):
        script = self.redq.broker.register_script(ZRANGEPOP)
        results = script(keys=[self.redq.eta_task_key], args=[int(round(time.time()))])
        for payload in results:
            payload = deserialize(payload)
            queue = payload.pop("queue")
            queue = Queue.create(queue, queues_by_name=self.redq.queues_by_name)
            payload = serialize(payload)
            self.redq.broker.lpush(queue.broker_key, payload)

    def _execute_next_task_from_queue(self):
        queues = [x.broker_key for x in self.redq.queues]
        print("Checking for tasks...")
        payload = self.redq.broker.blpop(queues, self.redq.wait_timeout)
        if payload is not None:
            queue, payload = payload
            payload = deserialize(payload)
            print(f"got task: {payload}")
            task = self.redq.tasks[payload["name"]]
